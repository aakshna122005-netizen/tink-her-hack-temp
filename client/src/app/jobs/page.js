...
import { IS_DEMO_MODE, DEMO_JOBS } from '@/lib/demo-data';
...
export default function JobsPage() {
    ...
    const fetchJobs = async () => {
        ...
        if (IS_DEMO_MODE) {
            ...
            if (remote === 'true') results = results.filter(j => j.remote);
            if (search) results = results.filter(j => j.title.toLowerCase().includes(search.toLowerCase()));
            setJobs(results);
            setTotalPages(1);
            setLoading(false);
            return;
        ...
        try {
            ...
            console.warn('API unavailable, loading demo jobs:', err.message);
            setJobs(DEMO_JOBS);
            setTotalPages(1);
        }
        setLoading(false);
