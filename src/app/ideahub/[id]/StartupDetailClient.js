...
export default function StartupDetailClient({ params }) {
    const { id } = use(params);
    const { user } = useAuth();
    ...
    useEffect(() => {
        if (IS_DEMO_MODE) {
            const found = DEMO_STARTUPS.find(s => s.id === id);
            setStartup(found || DEMO_STARTUPS[0]);
            setLoading(false);
            return;
        }
        api.get(`/ideahub/${id}`).then(({ data }) => setStartup(data.startup)).catch(() => {
            const found = DEMO_STARTUPS.find(s => s.id === id);
